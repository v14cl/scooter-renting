...
public class UpdateRentalCommandHandler {
    ...
    public RentalView handle(UpdateRentalCommand command) {
        var existingRental = rentalRepository.findById(command.rentalId())
                .orElseThrow(() -> new NotFoundException("Rental not found"));

        if (!command.actor().isAdmin() && !existingRental.renterId().equals(command.actor().userId())) {
            throw new AccessDeniedException("You cannot update this rental");
        }

        var updatedRental = rentalFactory.update(
                existingRental.id(),
