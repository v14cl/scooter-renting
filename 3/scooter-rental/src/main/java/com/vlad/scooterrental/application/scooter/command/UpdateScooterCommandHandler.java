...
public class UpdateScooterCommandHandler {
    ...
    public UpdateScooterCommandHandler(ScooterFactory scooterFactory, ScooterRepository scooterRepository) {
        this.scooterFactory = scooterFactory;
        this.scooterRepository = scooterRepository;
    }

    public ScooterView handle(UpdateScooterCommand command) {
        scooterRepository.findById(command.scooterId())
                .orElseThrow(() -> new NotFoundException("Scooter not found"));

        var scooter = scooterFactory.update(
                command.scooterId(),
                command.code(),
                command.model(),
                command.status(),
