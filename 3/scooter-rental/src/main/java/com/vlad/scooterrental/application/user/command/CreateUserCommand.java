...
public record CreateUserCommand(String fullName, String email, String password, Role role) {
    ...
    public static CreateUserCommand of(String fullName, String email, String password, String roleName) {
        if (roleName == null || roleName.isBlank()) {
            throw new ValidationException("Role must be provided");
        }
        try {
            return new CreateUserCommand(fullName, email, password, Role.valueOf(roleName.trim().toUpperCase()));
