...
public class RentalController {
    ...
    @PostMapping
    public ResponseEntity<RentalView> create(@Valid @RequestBody RentalRequest request, Authentication authentication) {
        RentalView rental = createRentalCommandHandler.handle(
    ...
    public List<RentalView> findAll(Authentication authentication) {
        return listRentalsQueryHandler.handle(SecurityUtils.actor(authentication));
    }

    @GetMapping("/{rentalId}")
    public RentalView findById(@PathVariable UUID rentalId, Authentication authentication) {
        return getRentalByIdQueryHandler.handle(rentalId, SecurityUtils.actor(authentication));
    }

    @PutMapping("/{rentalId}")
    public RentalView update(
            @PathVariable UUID rentalId,
            @Valid @RequestBody RentalRequest request,
            Authentication authentication
    ) {
        return updateRentalCommandHandler.handle(
    ...
    public ResponseEntity<Void> delete(@PathVariable UUID rentalId, Authentication authentication) {
        deleteRentalCommandHandler.handle(new DeleteRentalCommand(SecurityUtils.actor(authentication), rentalId));
