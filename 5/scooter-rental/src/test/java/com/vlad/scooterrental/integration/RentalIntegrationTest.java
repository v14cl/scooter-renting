...
class RentalIntegrationTest extends IntegrationTestSupport {
  ...
  void shouldCreateRentalAndHideOtherUsersRentals() throws Exception {
    ...
    mockMvc
        .perform(get("/api/rentals").header("Authorization", "Bearer " + secondCustomerToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isEmpty());

    mockMvc
        .perform(get("/api/rentals").header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
  ...
  void shouldUpdateRental() throws Exception {
    ...
    LocalDateTime startTime =
        LocalDateTime.now().plusDays(3).withHour(8).withMinute(0).withSecond(0).withNano(0);
    LocalDateTime endTime = startTime.plusHours(1);
    String createdRental =
        mockMvc
            .perform(
                post("/api/rentals")
                    .header("Authorization", "Bearer " + customerToken)
    ...
    mockMvc
        .perform(
            put("/api/rentals/{rentalId}", rentalId)
                .header("Authorization", "Bearer " + customerToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    json(
    ...
    mockMvc
        .perform(get("/api/rentals/{rentalId}", rentalId).header("Authorization", "Bearer " + customerToken))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.startTime")
                .value(
