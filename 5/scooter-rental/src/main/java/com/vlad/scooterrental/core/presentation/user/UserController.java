package com.vlad.scooterrental.core.presentation.user;

import com.vlad.scooterrental.core.application.user.command.CreateUserCommand;
import com.vlad.scooterrental.core.application.user.command.CreateUserCommandHandler;
import com.vlad.scooterrental.core.application.user.command.DeleteUserCommand;
import com.vlad.scooterrental.core.application.user.command.DeleteUserCommandHandler;
import com.vlad.scooterrental.core.application.user.command.UpdateUserCommand;
import com.vlad.scooterrental.core.application.user.command.UpdateUserCommandHandler;
import com.vlad.scooterrental.core.application.user.query.GetCurrentUserQuery;
import com.vlad.scooterrental.core.application.user.query.GetCurrentUserQueryHandler;
import com.vlad.scooterrental.core.application.user.query.GetUserByIdQuery;
import com.vlad.scooterrental.core.application.user.query.GetUserByIdQueryHandler;
import com.vlad.scooterrental.core.application.user.query.ListUsersQuery;
...
import com.vlad.scooterrental.core.presentation.dto.SecurityUtils;
import com.vlad.scooterrental.core.presentation.dto.UserCreateRequest;
import com.vlad.scooterrental.core.presentation.dto.UserUpdateRequest;
...
public class UserController {
  ...
  public ResponseEntity<CreatedIdResponse> create(@Valid @RequestBody UserCreateRequest request) {
    UUID userId =
        createUserCommandHandler.handle(
            new CreateUserCommand(
                request.fullName(), request.email(), request.password(), request.role()));
    return ResponseEntity.status(HttpStatus.CREATED).body(new CreatedIdResponse(userId));
  }

  @GetMapping
  @PreAuthorize("hasRole('ADMIN')")
  public List<UserView> findAll() {
    return listUsersQueryHandler.handle(new ListUsersQuery());
  }

  @GetMapping("/me")
  public UserView currentUser(Authentication authentication) {
    return getCurrentUserQueryHandler.handle(
        new GetCurrentUserQuery(SecurityUtils.userId(authentication)));
  }
  ...
  public ResponseEntity<Void> update(
    ...
    updateUserCommandHandler.handle(
        new UpdateUserCommand(
            userId, request.fullName(), request.email(), request.password(), request.role()));
