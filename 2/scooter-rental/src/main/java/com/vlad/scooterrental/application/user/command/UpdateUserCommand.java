...
public record UpdateUserCommand(
    UUID userId, String fullName, String email, String password, Role role) {
  public static UpdateUserCommand of(
      UUID userId, String fullName, String email, String password, String roleName) {
    if (roleName == null || roleName.isBlank()) {
      throw new ValidationException("Role must be provided");
    }
    try {
      return new UpdateUserCommand(
          userId, fullName, email, password, Role.valueOf(roleName.trim().toUpperCase()));
